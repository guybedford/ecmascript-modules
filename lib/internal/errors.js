...
E('ERR_INVALID_REPL_TYPE',
  'Cannot specify --format for REPL', TypeError);
...
E('ERR_INVALID_TYPE_FLAG',
  'Type flag must be one of "module", "commonjs". Received --format=%s',
  TypeError);
...
E('ERR_TYPE_MISMATCH', (filename, ext, formatFlag, conflict) => {
  const typeString =
    formatFlag === 'module' ? '--format=module or -m' : '--format=commonjs';
  // --format mismatches file extension
  if (conflict === 'extension')
    return `Extension ${ext} is not supported for ` +
      `${typeString} loading ${filename}`;
  // --format mismatches package.json "type"
